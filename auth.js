...
async function getSlots(cookies, ids) {
  ...
  console.log(resaUrl);
  resaUrl = 'https://planning.autocontrole.be/Reservaties/NieuwAutokeuringReservatie.aspx?VoertuigId=16e825e6-e99c-41d2-8461-4e1460dc080b&KlantId='+ids.klantId+'&KeuringsTypeId='+ids.keuringsTypeId
  const resaPage = await axios.get(resaUrl, { headers: { Cookie: cookies.join('; ') } });
  let $ = cheerio.load(resaPage.data);
